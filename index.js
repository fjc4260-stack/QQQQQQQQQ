...
async function pollForNewLines(page, collection) {
  ...
  try {
    ...
    console.error('[poll] خطا در خواندن متن صفحه:', err.message);
    return;
  }
  ...
  const newLines = lines.filter((line) => !seenLines.has(line));
...
async function main() {
  ...
  console.log('[main] پایش پیام‌ها آغاز شد. هر', POLL_INTERVAL_MS, 'میلی‌ثانیه بررسی می‌شود.');
}
